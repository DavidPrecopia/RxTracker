...
class TimeStampRepo(private val dao: TimeStampDao, private val timeUtil: ICurrentTimeUtil) :
    ...
    override fun getAll(): Flowable<List<TimeStamp>> = dao.getAll().map { mapDbTimeStamp(it) }

    override fun add(rxTitle: String): Completable =
        dao.add(DbTimeStamp(title = rxTitle, time = timeUtil.currentTime()))

    override fun delete(id: Int): Completable = dao.delete(id)
